...
void main()
{
    ...
    vector<vector<vector<lli> > > averaged_pixels(no_of_sectors,vector<vector<short> >(no_of_leds,vector<lli>(3,0)));
    ...
    for(int i=0;i<no_of_sectors;i++)
    {
    	...
    	{
    		lli cnt = pixels[i][j][3];
    		averaged_pixels[i][j][0]=(short)(pixels[i][j][0]/cnt); // averaged blue color
    		averaged_pixels[i][j][1]=(short)(pixels[i][j][1]/cnt); // averaged green color
    		averaged_pixels[i][j][2]=(short)(pixels[i][j][2]/cnt); // averaged red color
    	}
    }
...
void createResult(vector< vector< vector<lli> > > averaged_pixels)
{
